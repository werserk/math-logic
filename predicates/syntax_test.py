...
def test_term_substitute(debug=False):
    ...
    substitution_map = {
        "cd": Term.parse("g(f(xy))"),
        "xy": Term.parse("f(cd,xy)"),
        "_": Term.parse("d"),
    }
    result = Term.parse("h(cd,f(xy,_))").substitute(substitution_map, {"y"})
    if debug:
        print("Substituting", substitution_map, " in 'h(c,f(x, _))' gives", result)
    ...
    for substitution_map, forbidden_variables, variable_name in [
        ({"c": Term.parse("g(f(x))"), "x": Term.parse("f(c,y)")}, {"z", "y"}, "y"),
        ({"c": Term.parse("g(f(y))"), "x": Term.parse("f(c,x)")}, {"y", "z"}, "y"),
    ]:
        if debug:
            print(
                "Substituting",
...
def test_formula_variables(debug=False):
    for s, expected_variables in [
        ...
        ["(R(s(plus(times(zz,x),times(x,s(s(0))))))|Ex[Q(x,w)])", {"x", "zz", "w"}],
    ]:
...
def test_formula_functions(debug=False):
    for s, expected in [
        ...
        ["T(35,2,4)", set()],
        ["Q(25,x,minus(times(2,5)),s(3))", {("minus", 1), ("times", 2), ("s", 1)}],
        [
        ...
        ["Ax[T(35,x,4)]", set()],
        ["Ex[Q(25,x,minus(times(2,5)),s(3))]", {("minus", 1), ("times", 2), ("s", 1)}],
        [
...
def test_formula_substitute(debug=False):
    for s, substitution, expected in [
        ...
        formula = Formula.parse(s)
        substitution_map = {v: Term.parse(substitution[v]) for v in substitution.keys()}
        result = str(formula.substitute(substitution_map))
        if debug:
            print("Substituting", substitution_map, "in", formula, "yields", result)
    ...
    for s, substitution, forbidden_variables, variable_name in [
        ...
        try:
            result = str(formula.substitute(substitution_map, forbidden_variables))
            assert False, "Expected exception"
        except ForbiddenVariableError as e:
            if debug:
                print("Threw a ForbiddenVariableError as expected")
...
def test_propositional_skeleton(debug=False):
    ...
    for s, expected, expected_map in [
        ...
        if debug:
            print("The skeleton of", s, "is", skeleton, "with map", substitution_map)
