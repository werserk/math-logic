...
def test_instantiate_helper(debug=False):
    for (
        ...
        if debug:
            print(
                "Substituting constant and variable instantiation map",
                constant_and_variable_instantiation_map,
        ...
        result = schema._instantiate_helper(
            formula,
            frozendict(constant_and_variable_instantiation_map),
...
def test_instantiate(debug=False):
    for formula, templates, instantiation_map, instance in [
        ...
        (
            "(Ax[R(x)]->R(c))",
            {"x", "c", "R"},
            {"x": "z", "c": Term("z"), "R": Formula.parse("Ay[(L(x,y)->L(_,x))]")},
            "(Az[Ay[(L(x,y)->L(z,x))]]->Ay[(L(x,y)->L(z,x))])",
        ),
        (
            "(Ax[R(x)]->Ay[R(y)])",
        ...
        (
            "(c1=c2->(R(c1)->R(c2)))",
            {"c1", "c2", "R"},
            {"c1": Term("c2"), "c2": Term("c1"), "R": Formula.parse("R(c1,c2,_)")},
        ...
        (
            "(Ax[(Q()->R(x))]->(Q()->Ax[R(x)]))",
            {"R", "Q", "x"},
            {"Q": Formula.parse("v=0"), "R": Formula.parse("S(_)"), "x": "z"},
        ...
        (
            "(Axxx[RRR(xxx)]->Eyyy[QQQ(yyy)])",
            {"RRR", "yyy"},
            {"QQQ": Formula.parse("_=0")},
            None,
        ),
        (
            "(Axxx[RRR(xxx)]->Eyyy[QQQ(yyy)])",
            {"RRR", "yyy"},
            {"QQQ": Formula.parse("RRR(_)")},
            None,
        ),
        ...
        if debug:
            print(
                "Substituting instantiation map",
                instantiation_map,
                "in schema",
                schema,
                "...",
...
def test_mp_line_is_valid(debug=False):
    ...
    lines = [
        Proof.AssumptionLine(
            Formula.parse("(Ax[f(x)=0]->Ey[f(y)=1])"),
            assumption1,
...
def test_is_valid(debug=False):
    ...
    lines = [
        Proof.AssumptionLine(assumption1.formula, assumption1, {}),
...
def test_axiom_specialization_map_to_schema_instantiation_map(debug=False):
    from predicates.proofs import _axiom_specialization_map_to_schema_instantiation_map
    ...
    substitution_map = {
        "z1": Formula.parse("Ax[x=5]"),
        "z2": Formula.parse("M()"),
        "z3": Formula.parse("z2=5"),
    }
    expected = {
        "P": Formula.parse("(Ax[x=5]->M())"),
        "Q": Formula.parse("Ax[x=5]"),
        "R": Formula.parse("~(z2=5&M())"),
    }
    if debug:
        print(
            "Testing conversion of propositional specialization map",
            propositional_specialization_map,
            "to instantiation_map via substitution map",
            substitution_map,
        )
    instantiation_map = _axiom_specialization_map_to_schema_instantiation_map(
        frozendict(propositional_specialization_map), frozendict(substitution_map)
    )
...
def test_prove_from_skeleton_proof(debug=False):
    ...
    for formula, skeleton, substitution_map in [
        ...
        (
            ...
            {
                "z32": Formula.parse("Ey[~x=y]"),
                "z33": Formula.parse("y=3"),
