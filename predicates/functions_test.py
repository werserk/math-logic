    ...
    for s, valid_model, invalid_model in [
        ...
        for model, validity in [[valid_model, True], [invalid_model, False]]:
            ...
            if debug:
                print(
                    "which",
                    "is" if is_valid_model else "is not",
                    "satisfied by model",
                    model,
                )
...
def test_replace_functions_with_relations_in_formulas(debug=False):
    ...
    new_formulas = replace_functions_with_relations_in_formulas(frozenset({formula}))
    if debug:
...
def test_replace_equality_with_SAME_in_formulas(debug=False):
    ...
    if debug:
        print("... got", new_formulas)
    assert Formula.parse("Ax[Ay[Az[((S(x,y)&S(x,z))->SAME(y,z))]]]") in new_formulas
    ...
    for model, validity in [
        [
            Model(
                {"0", "1", "2"},
                {},
                {"S": set(), "SAME": {("0", "0"), ("1", "1"), ("2", "2"), ("0", "1")}},
            ),
            False,
        ],
        [
            Model({"0", "1", "2"}, {}, {"S": set(), "SAME": {("0", "0"), ("1", "1")}}),
            False,
        ],
        [
            Model(
