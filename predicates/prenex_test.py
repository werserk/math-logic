...
def test_pull_out_quantifications_across_binary_operator(debug=False):
    from predicates.prenex import _pull_out_quantifications_across_binary_operator
    ...
    for formula, expected in [
        ...
        if debug:
            print(
                "Testing _pull_out_quantifications_across_binary_operator on",
                formula,
                "...",
            )
...
def test_to_prenex_normal_form_from_uniquely_named_variables(debug=False):
    from predicates.prenex import _to_prenex_normal_form_from_uniquely_named_variables
    ...
    for formula, pnf in [
        ...
        if debug:
            print(
                "Testing " "_to_prenex_normal_form_from_uniquely_named_variables on",
                formula,
                "...",
            )
        result, proof = _to_prenex_normal_form_from_uniquely_named_variables(formula)
        if debug:
            print("... got", result)
...
def test_to_prenex_normal_form(debug=False):
    for formula, pnf in [
        ("Q(x,c)", "Q(x,c)"),
        ("Ax[Q(x,c)]", "Ax[Q(x,c)]"),
        ("~~(~Ax[Ey[R(x,y)]]&~Ax[Ey[x=y]])", "Ex[Ay[Eu[Aw[~~(~R(x,y)&~u=w)]]]]"),
        ("~~(~Ax[Ey[R(x,y)]]|~Ax[Ey[x=y]])", "Ex[Ay[Eu[Aw[~~(~R(x,y)|~u=w)]]]]"),
        ("~~(~Ax[Ey[R(x,y)]]->~Ax[Ey[x=y]])", "Ax[Ey[Eu[Aw[~~(~R(x,y)->~u=w)]]]]"),
