...
def test_evaluate_term(debug=False):
    ...
    assignment = {"x": "1", "y": "0"}
    for s, expected_value in [["x", "1"], ["plus(x,c)", "0"], ["plus(x,y)", "1"]]:
        term = Term.parse(s)
...
def test_evaluate_formula(debug=False):
    ...
    for assigned_value, expected_value in [("0", False), ("1", False), ("2", True)]:
        assignment = {"x": assigned_value}
        value = model.evaluate_formula(formula, frozendict(assignment))
        if debug:
            print("The value of", formula, "with assignment", assignment, "is", value)
        assert value == expected_value
...
def test_is_model_of(debug=False):
    ...
    for formulas, expected_result in [
        ...
        if debug:
            print("... is said", "" if result else "not", "to satisfy", formulas)
        assert result == expected_result

    formula = Formula.parse("(F(z,a)->z=b)")
    model = Model({"a", "b"}, {"a": "a", "b": "b"}, {"F": {("a", "a"), ("b", "b")}})
    if debug:
        print("The model", model, "...")
...
def __test_formula_evaluation_common(evaluate, tested_name, debug):
    ...
    for s, expected_value in [
        ...
        (
            "Ax[(x=0->Ax[(x=p1(0)->" "Ex[(x=0&Ax[((x=0|x=p1(0))|x=p1(p1(0)))])])])]",
            True,
        ),
    ]:
