...
def test_add_existential_derivation(debug=False):
    ...
    step3 = prover.add_universal_instantiation("(Man(y)->Mortal(y))", step1, "y")
    ...
    step2 = prover.add_universal_instantiation("(Man(y)->HumanityAlive())", step1, "y")
...
def test_add_flipped_equality(debug=False):
    ...
    assert _contains_line_with_formula(proof, "plus(x,plus(y,z))=plus(plus(x,y),z)")
...
def test_add_free_instantiation(debug=False):
    ...
    assert _contains_line_with_formula(proof, "plus(0,plus(x,0))=plus(plus(0,x),0)")
