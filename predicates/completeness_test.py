...
def _test_is_closed(
    test_primitively=False,
    test_universally=False,
    test_existentially=False,
    debug=False,
):
    ...
    for six_element_group_primitives in [
        ...
        _test_closures(
            six_element_group_primitives.union(SIX_ELEMENT_NEGATION_CLOSURE),
            True,
            True,
            True,
        )
    ...
    for six_element_group_primitives in [
        ...
        _test_closures(
            six_element_group_primitives - {Formula.parse("~Plus(4,3,5)")},
            False,
            True,
            True,
        )
        ...
        _test_closures(
            six_element_group_primitives - {Formula.parse("Plus(0,0,0)")},
            False,
            True,
            True,
        )
        ...
        _test_closures(
            six_element_group_primitives - {Formula.parse("~Plus(1,3,3)")},
            False,
            True,
            True,
        )
        ...
        _test_closures(
            (six_element_group_primitives - {Formula.parse("Plus(3,0,3)")}).union(
                {Formula.parse("~Plus(3,0,3)"), ZERO_AXIOM}
            ),
            True,
            False,
            True,
        ...
        _test_closures(
            six_element_group_primitives.union(
                {ZERO_AXIOM, NEGATION_AXIOM, COMMUTATIVITY_AXIOM},
                SIX_ELEMENT_NEGATION_CLOSURE - {Formula.parse("Ey[Plus(0,y,3)]")},
                SIX_ELEMENT_COMMUTATIVITY_CLOSURE,
            ),
            True,
            False,
...
def test_find_unsatisfied_quantifier_free_sentence(debug=False):
    ...
    if debug:
        print(
            "Testing find_unsatisfied_quantifier_free_sentence for the "
    ...
    assert not SIX_ELEMENT_COMMUTATIVE_GROUP_MODEL.evaluate_formula(quantifier_free)
...
def test_combine_contradictions(debug=False):
    ...
    if debug:
        print(
            "Testing combine_contradictions for the following two proofs:\n",
            proof_from_affirmation,
            "\n",
            proof_from_negation,
        )
    combined = combine_contradictions(proof_from_affirmation, proof_from_negation)
...
def test_eliminate_universal_instantiation_assumption(debug=False):
    ...
    if debug:
        print(
            "Verifying returned proof (" + "{:,}".format(len(eliminated.lines)),
            "lines) of",
...
def test_universal_closure_step(debug=False):
    ...
    assert augmented == {COMMUTATIVITY_AXIOM}.union(
        ...
        SIX_ELEMENT_COMMUTATIVITY_CLOSURE1,
    ...
    assert augmented == {COMMUTATIVITY_AXIOM}.union(
        ...
        SIX_ELEMENT_COMMUTATIVITY_CLOSURE1,
        SIX_ELEMENT_COMMUTATIVITY_CLOSURE2,
        SIX_ELEMENT_COMMUTATIVITY_CLOSURE3,
    )
    assert is_universally_closed(augmented)
    if debug:
        print("Testing universal_closure_step on result...")
...
def test_replace_constant(debug=False):
    ...
    from predicates.some_proofs_test import test_prove_group_unique_zero
...
def test_eliminate_existential_witness_assumption(debug=False):
    assumptions = {
        Schema(Formula.parse("Ax[(R(x)->(Q(x)&~Q(x)))]")),
        Schema(Formula.parse("Ex[R(x)]")),
...
ASSOCIATIVITY_AXIOM = Formula.parse(
    "Ax[Ay[Az[Axy[Ayz[Axyz[((Plus(xy,x,y)&Plus(yz,y,z))->"
...
SENTENCES_WITH_SIX_ELEMENTS = frozenset(
    {
        Formula.parse(s)
        for s in {
            "SAME(0,0)",
...
SIX_ELEMENT_ZERO_AXIOM_CLOSURE = frozenset(
    {
        Formula.parse(s)
        for s in {
            "Plus(0,0,0)",
...
SIX_ELEMENT_NEGATION_CLOSURE = frozenset(
    {
        Formula.parse(s)
        for s in {
            "Ey[Plus(0,y,0)]",
...
SIX_ELEMENT_COMMUTATIVITY_CLOSURE1 = frozenset(
    {
        Formula.parse(s)
        for s in {
            "Ay[Az[(Plus(z,0,y)->Plus(z,y,0))]]",
            "Ay[Az[(Plus(z,1,y)->Plus(z,y,1))]]",
            "Ay[Az[(Plus(z,2,y)->Plus(z,y,2))]]",
            "Ay[Az[(Plus(z,3,y)->Plus(z,y,3))]]",
            "Ay[Az[(Plus(z,4,y)->Plus(z,y,4))]]",
            "Ay[Az[(Plus(z,5,y)->Plus(z,y,5))]]",
        }
    }
)
SIX_ELEMENT_COMMUTATIVITY_CLOSURE2 = frozenset(
    {
        Formula.parse(s)
        for s in {
            "Az[(Plus(z,0,0)->Plus(z,0,0))]",
            "Az[(Plus(z,0,1)->Plus(z,1,0))]",
            "Az[(Plus(z,0,2)->Plus(z,2,0))]",
            "Az[(Plus(z,0,3)->Plus(z,3,0))]",
            "Az[(Plus(z,0,4)->Plus(z,4,0))]",
            "Az[(Plus(z,0,5)->Plus(z,5,0))]",
            "Az[(Plus(z,1,0)->Plus(z,0,1))]",
            "Az[(Plus(z,1,1)->Plus(z,1,1))]",
            "Az[(Plus(z,1,2)->Plus(z,2,1))]",
            "Az[(Plus(z,1,3)->Plus(z,3,1))]",
            "Az[(Plus(z,1,4)->Plus(z,4,1))]",
            "Az[(Plus(z,1,5)->Plus(z,5,1))]",
            "Az[(Plus(z,2,0)->Plus(z,0,2))]",
            "Az[(Plus(z,2,1)->Plus(z,1,2))]",
            "Az[(Plus(z,2,2)->Plus(z,2,2))]",
            "Az[(Plus(z,2,3)->Plus(z,3,2))]",
            "Az[(Plus(z,2,4)->Plus(z,4,2))]",
            "Az[(Plus(z,2,5)->Plus(z,5,2))]",
            "Az[(Plus(z,3,0)->Plus(z,0,3))]",
            "Az[(Plus(z,3,1)->Plus(z,1,3))]",
            "Az[(Plus(z,3,2)->Plus(z,2,3))]",
            "Az[(Plus(z,3,3)->Plus(z,3,3))]",
            "Az[(Plus(z,3,4)->Plus(z,4,3))]",
            "Az[(Plus(z,3,5)->Plus(z,5,3))]",
            "Az[(Plus(z,4,0)->Plus(z,0,4))]",
            "Az[(Plus(z,4,1)->Plus(z,1,4))]",
            "Az[(Plus(z,4,2)->Plus(z,2,4))]",
            "Az[(Plus(z,4,3)->Plus(z,3,4))]",
            "Az[(Plus(z,4,4)->Plus(z,4,4))]",
            "Az[(Plus(z,4,5)->Plus(z,5,4))]",
            "Az[(Plus(z,5,0)->Plus(z,0,5))]",
            "Az[(Plus(z,5,1)->Plus(z,1,5))]",
            "Az[(Plus(z,5,2)->Plus(z,2,5))]",
            "Az[(Plus(z,5,3)->Plus(z,3,5))]",
            "Az[(Plus(z,5,4)->Plus(z,4,5))]",
            "Az[(Plus(z,5,5)->Plus(z,5,5))]",
        }
    }
)
SIX_ELEMENT_COMMUTATIVITY_CLOSURE3 = frozenset(
    {
        Formula.parse(s)
        for s in {
            "(Plus(0,0,0)->Plus(0,0,0))",
            "(Plus(1,0,0)->Plus(1,0,0))",
            "(Plus(2,0,0)->Plus(2,0,0))",
            "(Plus(3,0,0)->Plus(3,0,0))",
            "(Plus(4,0,0)->Plus(4,0,0))",
            "(Plus(5,0,0)->Plus(5,0,0))",
            "(Plus(0,0,1)->Plus(0,1,0))",
            "(Plus(1,0,1)->Plus(1,1,0))",
            "(Plus(2,0,1)->Plus(2,1,0))",
            "(Plus(3,0,1)->Plus(3,1,0))",
            "(Plus(4,0,1)->Plus(4,1,0))",
            "(Plus(5,0,1)->Plus(5,1,0))",
            "(Plus(0,0,2)->Plus(0,2,0))",
            "(Plus(1,0,2)->Plus(1,2,0))",
            "(Plus(2,0,2)->Plus(2,2,0))",
            "(Plus(3,0,2)->Plus(3,2,0))",
            "(Plus(4,0,2)->Plus(4,2,0))",
            "(Plus(5,0,2)->Plus(5,2,0))",
            "(Plus(0,0,3)->Plus(0,3,0))",
            "(Plus(1,0,3)->Plus(1,3,0))",
            "(Plus(2,0,3)->Plus(2,3,0))",
            "(Plus(3,0,3)->Plus(3,3,0))",
            "(Plus(4,0,3)->Plus(4,3,0))",
            "(Plus(5,0,3)->Plus(5,3,0))",
            "(Plus(0,0,4)->Plus(0,4,0))",
            "(Plus(1,0,4)->Plus(1,4,0))",
            "(Plus(2,0,4)->Plus(2,4,0))",
            "(Plus(3,0,4)->Plus(3,4,0))",
            "(Plus(4,0,4)->Plus(4,4,0))",
            "(Plus(5,0,4)->Plus(5,4,0))",
            "(Plus(0,0,5)->Plus(0,5,0))",
            "(Plus(1,0,5)->Plus(1,5,0))",
            "(Plus(2,0,5)->Plus(2,5,0))",
            "(Plus(3,0,5)->Plus(3,5,0))",
            "(Plus(4,0,5)->Plus(4,5,0))",
            "(Plus(5,0,5)->Plus(5,5,0))",
            "(Plus(0,1,0)->Plus(0,0,1))",
            "(Plus(1,1,0)->Plus(1,0,1))",
            "(Plus(2,1,0)->Plus(2,0,1))",
            "(Plus(3,1,0)->Plus(3,0,1))",
            "(Plus(4,1,0)->Plus(4,0,1))",
            "(Plus(5,1,0)->Plus(5,0,1))",
            "(Plus(0,1,1)->Plus(0,1,1))",
            "(Plus(1,1,1)->Plus(1,1,1))",
            "(Plus(2,1,1)->Plus(2,1,1))",
            "(Plus(3,1,1)->Plus(3,1,1))",
            "(Plus(4,1,1)->Plus(4,1,1))",
            "(Plus(5,1,1)->Plus(5,1,1))",
            "(Plus(0,1,2)->Plus(0,2,1))",
            "(Plus(1,1,2)->Plus(1,2,1))",
            "(Plus(2,1,2)->Plus(2,2,1))",
            "(Plus(3,1,2)->Plus(3,2,1))",
            "(Plus(4,1,2)->Plus(4,2,1))",
            "(Plus(5,1,2)->Plus(5,2,1))",
            "(Plus(0,1,3)->Plus(0,3,1))",
            "(Plus(1,1,3)->Plus(1,3,1))",
            "(Plus(2,1,3)->Plus(2,3,1))",
            "(Plus(3,1,3)->Plus(3,3,1))",
            "(Plus(4,1,3)->Plus(4,3,1))",
            "(Plus(5,1,3)->Plus(5,3,1))",
            "(Plus(0,1,4)->Plus(0,4,1))",
            "(Plus(1,1,4)->Plus(1,4,1))",
            "(Plus(2,1,4)->Plus(2,4,1))",
            "(Plus(3,1,4)->Plus(3,4,1))",
            "(Plus(4,1,4)->Plus(4,4,1))",
            "(Plus(5,1,4)->Plus(5,4,1))",
            "(Plus(0,1,5)->Plus(0,5,1))",
            "(Plus(1,1,5)->Plus(1,5,1))",
            "(Plus(2,1,5)->Plus(2,5,1))",
            "(Plus(3,1,5)->Plus(3,5,1))",
            "(Plus(4,1,5)->Plus(4,5,1))",
            "(Plus(5,1,5)->Plus(5,5,1))",
            "(Plus(0,2,0)->Plus(0,0,2))",
            "(Plus(1,2,0)->Plus(1,0,2))",
            "(Plus(2,2,0)->Plus(2,0,2))",
            "(Plus(3,2,0)->Plus(3,0,2))",
            "(Plus(4,2,0)->Plus(4,0,2))",
            "(Plus(5,2,0)->Plus(5,0,2))",
            "(Plus(0,2,1)->Plus(0,1,2))",
            "(Plus(1,2,1)->Plus(1,1,2))",
            "(Plus(2,2,1)->Plus(2,1,2))",
            "(Plus(3,2,1)->Plus(3,1,2))",
            "(Plus(4,2,1)->Plus(4,1,2))",
            "(Plus(5,2,1)->Plus(5,1,2))",
            "(Plus(0,2,2)->Plus(0,2,2))",
            "(Plus(1,2,2)->Plus(1,2,2))",
            "(Plus(2,2,2)->Plus(2,2,2))",
            "(Plus(3,2,2)->Plus(3,2,2))",
            "(Plus(4,2,2)->Plus(4,2,2))",
            "(Plus(5,2,2)->Plus(5,2,2))",
            "(Plus(0,2,3)->Plus(0,3,2))",
            "(Plus(1,2,3)->Plus(1,3,2))",
            "(Plus(2,2,3)->Plus(2,3,2))",
            "(Plus(3,2,3)->Plus(3,3,2))",
            "(Plus(4,2,3)->Plus(4,3,2))",
            "(Plus(5,2,3)->Plus(5,3,2))",
            "(Plus(0,2,4)->Plus(0,4,2))",
            "(Plus(1,2,4)->Plus(1,4,2))",
            "(Plus(2,2,4)->Plus(2,4,2))",
            "(Plus(3,2,4)->Plus(3,4,2))",
            "(Plus(4,2,4)->Plus(4,4,2))",
            "(Plus(5,2,4)->Plus(5,4,2))",
            "(Plus(0,2,5)->Plus(0,5,2))",
            "(Plus(1,2,5)->Plus(1,5,2))",
            "(Plus(2,2,5)->Plus(2,5,2))",
            "(Plus(3,2,5)->Plus(3,5,2))",
            "(Plus(4,2,5)->Plus(4,5,2))",
            "(Plus(5,2,5)->Plus(5,5,2))",
            "(Plus(0,3,0)->Plus(0,0,3))",
            "(Plus(1,3,0)->Plus(1,0,3))",
            "(Plus(2,3,0)->Plus(2,0,3))",
            "(Plus(3,3,0)->Plus(3,0,3))",
            "(Plus(4,3,0)->Plus(4,0,3))",
            "(Plus(5,3,0)->Plus(5,0,3))",
            "(Plus(0,3,1)->Plus(0,1,3))",
            "(Plus(1,3,1)->Plus(1,1,3))",
            "(Plus(2,3,1)->Plus(2,1,3))",
            "(Plus(3,3,1)->Plus(3,1,3))",
            "(Plus(4,3,1)->Plus(4,1,3))",
            "(Plus(5,3,1)->Plus(5,1,3))",
            "(Plus(0,3,2)->Plus(0,2,3))",
            "(Plus(1,3,2)->Plus(1,2,3))",
            "(Plus(2,3,2)->Plus(2,2,3))",
            "(Plus(3,3,2)->Plus(3,2,3))",
            "(Plus(4,3,2)->Plus(4,2,3))",
            "(Plus(5,3,2)->Plus(5,2,3))",
            "(Plus(0,3,3)->Plus(0,3,3))",
            "(Plus(1,3,3)->Plus(1,3,3))",
            "(Plus(2,3,3)->Plus(2,3,3))",
            "(Plus(3,3,3)->Plus(3,3,3))",
            "(Plus(4,3,3)->Plus(4,3,3))",
            "(Plus(5,3,3)->Plus(5,3,3))",
            "(Plus(0,3,4)->Plus(0,4,3))",
            "(Plus(1,3,4)->Plus(1,4,3))",
            "(Plus(2,3,4)->Plus(2,4,3))",
            "(Plus(3,3,4)->Plus(3,4,3))",
            "(Plus(4,3,4)->Plus(4,4,3))",
            "(Plus(5,3,4)->Plus(5,4,3))",
            "(Plus(0,3,5)->Plus(0,5,3))",
            "(Plus(1,3,5)->Plus(1,5,3))",
            "(Plus(2,3,5)->Plus(2,5,3))",
            "(Plus(3,3,5)->Plus(3,5,3))",
            "(Plus(4,3,5)->Plus(4,5,3))",
            "(Plus(5,3,5)->Plus(5,5,3))",
            "(Plus(0,4,0)->Plus(0,0,4))",
            "(Plus(1,4,0)->Plus(1,0,4))",
            "(Plus(2,4,0)->Plus(2,0,4))",
            "(Plus(3,4,0)->Plus(3,0,4))",
            "(Plus(4,4,0)->Plus(4,0,4))",
            "(Plus(5,4,0)->Plus(5,0,4))",
            "(Plus(0,4,1)->Plus(0,1,4))",
            "(Plus(1,4,1)->Plus(1,1,4))",
            "(Plus(2,4,1)->Plus(2,1,4))",
            "(Plus(3,4,1)->Plus(3,1,4))",
            "(Plus(4,4,1)->Plus(4,1,4))",
            "(Plus(5,4,1)->Plus(5,1,4))",
            "(Plus(0,4,2)->Plus(0,2,4))",
            "(Plus(1,4,2)->Plus(1,2,4))",
            "(Plus(2,4,2)->Plus(2,2,4))",
            "(Plus(3,4,2)->Plus(3,2,4))",
            "(Plus(4,4,2)->Plus(4,2,4))",
            "(Plus(5,4,2)->Plus(5,2,4))",
            "(Plus(0,4,3)->Plus(0,3,4))",
            "(Plus(1,4,3)->Plus(1,3,4))",
            "(Plus(2,4,3)->Plus(2,3,4))",
            "(Plus(3,4,3)->Plus(3,3,4))",
            "(Plus(4,4,3)->Plus(4,3,4))",
            "(Plus(5,4,3)->Plus(5,3,4))",
            "(Plus(0,4,4)->Plus(0,4,4))",
            "(Plus(1,4,4)->Plus(1,4,4))",
            "(Plus(2,4,4)->Plus(2,4,4))",
            "(Plus(3,4,4)->Plus(3,4,4))",
            "(Plus(4,4,4)->Plus(4,4,4))",
            "(Plus(5,4,4)->Plus(5,4,4))",
            "(Plus(0,4,5)->Plus(0,5,4))",
            "(Plus(1,4,5)->Plus(1,5,4))",
            "(Plus(2,4,5)->Plus(2,5,4))",
            "(Plus(3,4,5)->Plus(3,5,4))",
            "(Plus(4,4,5)->Plus(4,5,4))",
            "(Plus(5,4,5)->Plus(5,5,4))",
            "(Plus(0,5,0)->Plus(0,0,5))",
            "(Plus(1,5,0)->Plus(1,0,5))",
            "(Plus(2,5,0)->Plus(2,0,5))",
            "(Plus(3,5,0)->Plus(3,0,5))",
            "(Plus(4,5,0)->Plus(4,0,5))",
            "(Plus(5,5,0)->Plus(5,0,5))",
            "(Plus(0,5,1)->Plus(0,1,5))",
            "(Plus(1,5,1)->Plus(1,1,5))",
            "(Plus(2,5,1)->Plus(2,1,5))",
            "(Plus(3,5,1)->Plus(3,1,5))",
            "(Plus(4,5,1)->Plus(4,1,5))",
            "(Plus(5,5,1)->Plus(5,1,5))",
            "(Plus(0,5,2)->Plus(0,2,5))",
            "(Plus(1,5,2)->Plus(1,2,5))",
            "(Plus(2,5,2)->Plus(2,2,5))",
            "(Plus(3,5,2)->Plus(3,2,5))",
            "(Plus(4,5,2)->Plus(4,2,5))",
            "(Plus(5,5,2)->Plus(5,2,5))",
            "(Plus(0,5,3)->Plus(0,3,5))",
            "(Plus(1,5,3)->Plus(1,3,5))",
            "(Plus(2,5,3)->Plus(2,3,5))",
            "(Plus(3,5,3)->Plus(3,3,5))",
            "(Plus(4,5,3)->Plus(4,3,5))",
            "(Plus(5,5,3)->Plus(5,3,5))",
            "(Plus(0,5,4)->Plus(0,4,5))",
            "(Plus(1,5,4)->Plus(1,4,5))",
            "(Plus(2,5,4)->Plus(2,4,5))",
            "(Plus(3,5,4)->Plus(3,4,5))",
            "(Plus(4,5,4)->Plus(4,4,5))",
            "(Plus(5,5,4)->Plus(5,4,5))",
            "(Plus(0,5,5)->Plus(0,5,5))",
            "(Plus(1,5,5)->Plus(1,5,5))",
            "(Plus(2,5,5)->Plus(2,5,5))",
            "(Plus(3,5,5)->Plus(3,5,5))",
            "(Plus(4,5,5)->Plus(4,5,5))",
            "(Plus(5,5,5)->Plus(5,5,5))",
        }
    }
)
