...
def test_remove_assumption(debug=False):
    ...
    for proof, assumption1, assumption2 in [
        ...
        if debug:
            print(
                "Testing remove_assumption with assumption '"
                + assumption1
        ...
        assert result1.conclusion == Formula("->", assumption1, proof.conclusion)
...
def test_prove_by_way_of_contradiction(debug=False):
    ...
    from predicates.some_proofs import COMPREHENSION_AXIOM, prove_russell_paradox
    from predicates.some_proofs_test import test_prove_russell_paradox

    test_prove_russell_paradox()
    assumption = Formula.parse("Ey[Ax[((In(x,y)->~In(x,x))&(~In(x,x)->In(x,y)))]]")
    ...
    if debug:
        print(
            "Testing prove_by_way_of_contradiction with assumption '"
            + str(assumption)
            + "' for the following proof:\n"
            + str(new_proof)
        )
