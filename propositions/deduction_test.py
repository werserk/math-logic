...
R8 = InferenceRule([Formula.parse("p"), Formula.parse("q")], Formula.parse("(p&q)"))
TRI_AND_PROOF = Proof(
    InferenceRule(
...
def test_prove_from_opposites(debug=False):
    assumptions = (Formula.parse("(~~p->~~q)"), Formula.parse("p"), Formula.parse("~q"))
    rules = {MP, I1, N, NI}
    pf = Proof(
        InferenceRule(assumptions, Formula.parse("(q->p)")),
        rules,
        [
            Proof.Line(Formula.parse("p")),
    ...
    if debug:
        print(
            "Testing prove_from_opposites with assumptions",
            assumptions,
            "and conclusion",
            g,
        )
...
def test_prove_by_way_of_contradiction(debug=False):
    assumptions = (Formula.parse("(~r->p)"), Formula.parse("~p"), Formula.parse("~r"))
    p = Proof(
        InferenceRule(assumptions, Formula.parse("~(p->p)")),
        {MP, NI},
        [
            Proof.Line(Formula.parse("~r")),
