...
def test_reduce_assumption(debug=False):
    for f, m, v in [
        ...
        if debug:
            print("testing reduce assumption on", pt.statement, "and", pf.statement)
...
def test_encode_as_formula(debug=False):
    for r, f in [
        (InferenceRule([Formula.parse("x")], Formula.parse("y")), "(x->y)"),
        (
            InferenceRule([Formula.parse("x"), Formula.parse("y")], Formula.parse("z")),
            "(x->(y->z))",
        ),
        (
        ...
        (
            InferenceRule(
                [Formula.parse("~p"), Formula.parse("(q->p)"), Formula.parse("z")],
                Formula.parse("~q"),
            ),
            "(~p->((q->p)->(z->~q)))",
        ),
